...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		customTransIn = new ScreenWipeIn(0.3);
		customTransOut = new ScreenWipeOut(0.3);
		...
		gf = new Character(400 * 3, 130 * 3, gfCheck);
		gf.scrollFactor.set(0.95, 0.95);

		var dadChar = SONG.player2;

		dad = new Character(100 * 3, 100 * 3, dadChar);

		var bfChar = SONG.player1;

		boyfriend = new Boyfriend(770 * 3, 450 * 3, bfChar);
		...
		else
		{
			defaultCamZoom = 0.9;
			curStage = 'stage';
			var bg:FlxSprite = new FlxSprite(-1800, -600);
			bg.frames = Paths.getSparrowAtlasFunk("stagestuff");
			bg.antialiasing = true;
			bg.scrollFactor.set(0.9, 0.9);

			bg.animation.addByPrefix("stageback2", "stageback2", 0, false);
			bg.animation.addByPrefix("stagecurtains", "stagecurtains", 0, false);
			bg.animation.play("stageback2");
			bg.updateHitbox();
			add(bg);

			var stageCurtains = bg.clone();
			stageCurtains.setPosition(-1500, -900);
			stageCurtains.animation.play("stagecurtains");
		...
		{
			comboUI = new ComboPopup(boyfriend.x - 250, boyfriend.y - 75, [Paths.image("ratings"), 403 * 3, 163 * 3, true],
				[Paths.image("numbers"), 100 * 3, 120 * 3, true], [Paths.image("comboBreak"), 348 * 3, 211 * 3, true]);
		}
		else
		{
		...
		if (Config.downscroll)
		{
			strumLine = new FlxSprite(0, FlxG.height - Note.swagWidth * 1.5).makeGraphic(1, 1);
		}
		else
		{
		...
		healthBar = new FlxBar(healthBarBG.x + 18, healthBarBG.y + 15, RIGHT_TO_LEFT, Std.int(healthBarBG.width - 36), Std.int(healthBarBG.height - 30), this,
			'health', 0, 2);
		...
		var bgDim = new FlxSprite(0, 0).makeGraphic(1, 1, FlxColor.BLACK);
	...
	function updateNote()
	{
		notes.forEachAlive(function(daNote:Note)
		{
			...
			if (Config.downscroll)
			{
				daNote.y = (strumLine.y + (Conductor.songPosition - daNote.strumTime) * (0.45 * 3 * FlxMath.roundDecimal(PlayState.SONG.speed, 2)));
